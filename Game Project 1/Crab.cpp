...
Crab::Crab(Vector2f  position, std::shared_ptr<Texture> sprite, EnemyManager* manager, int tilesize, float speed, int power)
{
...
Crab::Crab(Vector2f  position, std::string pathtosprite, EnemyManager* manager, int tilesize, float speed, int power)
{
	m_pEnemyManager = manager;
	m_LastPosition = m_DrawPosition = m_Position = position;
	m_Speed = speed;
...
Crab::Crab(Crab && other) : Crab(other)
{
}

Crab & Crab::operator=(const Crab & other)
{
	m_AccumulatedSeconds = 0;
	m_Routine = {};
	m_InvincabilityTime = 0;
...
void Crab::Update(const float elapsedseconds, Map* stage, Player* target, bool recalculate)
{
...
void Crab::Draw(const float interpolate)
{
...
void Crab::Hit(Sutils::Direction direction)
{
	if (!m_IsHit)
	{
		Move(direction);
...
void Crab::Attack(Player* caster)
{

}

void Crab::Move(Sutils::Direction direction)
{
	switch (direction)
	{
...
void Crab::SetState(Sutils::Direction state)
{
...
void Crab::Translate(Vector2f movement)
{
	m_Position += movement;
}

Vector2f Crab::GetSpriteCenter() const
{
...
void Crab::InitRoutine(std::vector<Vector2f> route, Map& stage)
{
